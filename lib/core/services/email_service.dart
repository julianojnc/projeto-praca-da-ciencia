...
class EmailService {
  ...
  static Future<void> sendConfirmationToVisitor({
    ...
    final formattedCreatedAt = DateFormat('dd/MM/yyyy HH:mm').format(DateTime.now());

    final smtpServer = gmail(_senderEmail, _senderPassword);

    final message = Message()
      ..from = Address(_senderEmail, 'PraçaDaCiência')
      ..recipients.add(visitorEmail)
      ..subject = 'Agendamento Confirmado - $visitorName - $formattedDate $selectedTime'
      ..html = """
  ...
  // Envia e-mail de notificação para a Praça da Ciência
  static Future<void> sendNotificationToPraca({
    required String visitorName,
    required String visitorEmail,
    ...
    final message = Message()
      ...
      ..subject = 'Novo Agendamento - $visitorName - $formattedDate $selectedTime'
      ..html = """
